...
class Transformer_Attention(torch.nn.Module):
    def __init__(self, in_channels, out_channels, heads=8, dropout=0.1, concat=True, beta=False):
        super(Transformer_Attention, self).__init__()
        self.transformer = TransformerConv(in_channels, out_channels, heads=heads, dropout=dropout, concat=concat, beta=beta)
...
class GATGNN(torch.nn.Module):
    def __init__(self,heads,classification=None,neurons=64,nl=3,xtra_layers=True,global_attention='composition',
        ...
        # For AGAT comment these lines (193-194)
    ...
    def forward(self,data):
        ...
        # For AGAT comment this block (243-250)
        if   self.g_a in ['cluster', 'unpooling', 'clustering']:
            ...
            x         = (x)*ag
        elif self.g_a == 'transformer':  # GI-M5
            x = self.transformer_att(x, edge_index)
