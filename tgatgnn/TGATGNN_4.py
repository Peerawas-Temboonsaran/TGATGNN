...
class GATGNN(torch.nn.Module):
    def __init__(self,heads,transformer_heads,classification=None,neurons=64,nl=3,xtra_layers=True,global_attention='composition',
                 unpooling_technique='random',concat_comp=False,edge_format='CGCNN', num_transformer_layers=2):
        super(GATGNN, self).__init__()

        self.n_heads        = heads
        self.t_heads        = transformer_heads
        self.classification = True if classification != None else False 
        # For AGAT comment these lines (182-183)
        ...
        # For AGAT comment these lines (200-201)
        ...
        self.num_transformer_layers = num_transformer_layers  # Store the number of transformer layers
        self.transformer_layers = nn.ModuleList([TransformerLayer(reg_h, self.t_heads) for _ in range(num_transformer_layers)])  # Initialize Transformer Layers
    ...
    def forward(self,data):
        ...
        for a_idx in range(len(self.node_att)):
            ...
            # Store embeddings after each layer
            layer_embeddings.append(x.clone().detach())

        # For AGAT comment this block (231-236)
        ...
        return y, layer_embeddings  # Assuming you still want to return layer_embeddings
